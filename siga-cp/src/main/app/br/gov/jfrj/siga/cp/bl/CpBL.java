...
public class CpBL {
	...
	public String alterarSenha(String cpf, String email, String matricula, String baseUrl)
				...
				if(!lista.isEmpty()) {
					...
					for (CpIdentidade cpIdentidade : lista) {
						Cp.getInstance().getBL().alterarSenhaDeIdentidade(cpIdentidade.getNmLoginIdentidade(),
								StringUtils.leftPad(cpIdentidade.getDpPessoa().getCpfPessoa().toString(), 11, "0"), null, senhaGerada, baseUrl);
					}
					resultado = "OK";
				} else {
	...
	public CpIdentidade alterarSenhaDeIdentidade(String matricula, String cpf,
			CpIdentidade idCadastrante, String[] senhaGerada, String baseUrl)
			throws AplicacaoException {
			...
			if (id != null) {
				...
				try {
					...
					if (SigaMessages.isSigaSP()) {
						...
						String ambiente = Cp.getInstance().getProp().obterPropriedade("ambiente");
						String corpo = textoEmailUsuario(idNova, novaSenha, autenticaPeloBanco, CpAmbienteEnumBL.pelaPrimeiraLetra(ambiente), baseUrl);
						Correio.enviar(pessoa.getEmailPessoaAtual(), "[" + ambiente.toUpperCase() + "] Alteração de Senha de Acesso", corpo);
					}
	...
	public CpIdentidade criarIdentidade(String matricula, String cpf,
			...
			String[] senhaGerada, boolean marcarParaSinc, String baseUrl)
			throws AplicacaoException {
				...
				if (pessoa.getEmailPessoaAtual() != null) {
					...
					try {
						...
						if (SigaMessages.isSigaSP()) {
							...
							String ambiente = Cp.getInstance().getProp().obterPropriedade("ambiente");
							String corpo = textoEmailUsuario(idNova, novaSenha, autenticaPeloBanco, CpAmbienteEnumBL.pelaPrimeiraLetra(ambiente), baseUrl);
							Correio.enviar(pessoa.getEmailPessoaAtual(), "[" + ambiente.toUpperCase() + "] Novo Usuário", corpo);
						}
						dao().commitTransacao();
						return idNova;
					} catch (final Exception e) {
    ...
    private String textoEmailUsuario(CpIdentidade identidade, String novaSenha, boolean autenticaPeloBanco, CpAmbienteEnumBL ambiente, String requestUrl) {
    	DpPessoa pessoa = identidade.getDpPessoa();

    	StringBuilder mensagem = new StringBuilder()
				.append("CPF Usuário: ")
				.append(pessoa.getCpfFormatado())
				.append("\nSenha: ");

		if (autenticaPeloBanco) {
			mensagem.append(novaSenha);
		}	else {
			mensagem.append("a mesma usada para logon na rede (Windows).");
		}

		mensagem.append("\nCódigo: ")
				.append(pessoa.getSigla())
				.append("\n\nAmbiente ")
				.append(ambiente.name())
				.append(", disponível para acesso em ")
				.append(requestUrl);

		return mensagem.append("\n\n")
				.append("Atenção: esta é uma mensagem automática, por favor não responda.")
				.toString();
	}
...
//						.criptografar(senhaNova, chave));
	...
	public InputStream uploadFuncao(InputStream sourceInputStream, CpOrgaoUsuario orgaoUsuario, String extensao) {
		InputStream inputStream = null;
		try {
			Excel excel = new Excel();
			inputStream = excel.uploadFuncao(sourceInputStream, orgaoUsuario, extensao);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return inputStream;
	...
	public InputStream uploadCargo(InputStream sourceInputStream, CpOrgaoUsuario orgaoUsuario, String extensao) {
		InputStream inputStream = null;
		try {
			Excel excel = new Excel();
			inputStream = excel.uploadCargo(sourceInputStream, orgaoUsuario, extensao);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return inputStream;
