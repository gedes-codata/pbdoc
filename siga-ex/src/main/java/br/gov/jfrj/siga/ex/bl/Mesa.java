...
public class Mesa {
	...
	private static final Comparator<MesaItem> MESA_COMPARATOR = (MesaItem o1, MesaItem o2) -> {
		int index = Integer.compare(parseInt(o1.grupoOrdem), parseInt(o2.grupoOrdem));
		if (index != 0) {
			return index;
		}
		index = o2.datahora.compareTo(o1.datahora);
		if (index != 0) {
			return index;
		}
		return 0;
	...
	private static final String TIPO_MESA_DOCUMENTO = "Documento";
	...
	private static final int PADRAO_MAXIMO_DIAS_DEVOLUCAO_ATRASO = 5;

	private static final long UM_MINUTO_MILISSEG = 1000l * 60l;
	private static final long UMA_HORA_MILISSEG = UM_MINUTO_MILISSEG * 60l;
	private static final long UM_DIA_MILISSEG = UMA_HORA_MILISSEG * 24l;
	...
	public static SortedSet<MesaItem> listarReferencias(TipoDePainelEnum tipo,
			Map<ExMobil, List<MeM>> references, DpPessoa pessoa,
			DpLotacao unidade, Date currentDate) {

		// new Timestamp(System.currentTimeMillis());
		Date dataHoje = new Date();
		SortedSet<MesaItem> itensMesa = new TreeSet<>(MESA_COMPARATOR);

		for (ExMobil mobil : references.keySet()) {
			MesaItem r = new MesaItem();
			r.tipo = TIPO_MESA_DOCUMENTO;
			...
			r.descr = mobil.getDoc().getDescrCurta(80);

			if (mobil.getDoc().getSubscritor() != null && mobil.getDoc().getSubscritor().getLotacao() != null) {
				if (SigaMessages.isSigaSP()) {
					r.origem = mobil.getDoc().getSubscritor().getLotacao().getOrgaoUsuario()
							+ "/" + mobil.getDoc().getSubscritor().getLotacao().getSigla();
				} else {
					r.origem = mobil.getDoc().getSubscritor().getLotacao().getSigla();
				}
			}

			r.destino = ofNullable(mobil.getDoc())
			...
			if (mobil.getUltimaMovimentacaoNaoCancelada() != null
				...
				Date dataMovimentacao = mobil.getUltimaMovimentacaoNaoCancelada().getDtFimMov();
				long dias;
				if (dataHoje.compareTo(dataMovimentacao) <= 0) {
					dias = (dataMovimentacao.getTime() - dataHoje.getTime() - UMA_HORA_MILISSEG) / UM_DIA_MILISSEG;

					String paramDiasDevolucao = stripToNull(SigaBaseProperties.getString("siga.qtdDiasDevolucao"));
					long maximoDiasDevolucaoAtraso = paramDiasDevolucao != null ? Long.valueOf(paramDiasDevolucao) : PADRAO_MAXIMO_DIAS_DEVOLUCAO_ATRASO;
					if (dias <= maximoDiasDevolucaoAtraso) {
	...
	public static SortedSet<MesaItem> getMesa(ExDao dao, DpPessoa titular,
