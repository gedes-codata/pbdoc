	...
	<c:if test="${not empty url}">
		<c:choose>
			<c:when test="${not empty popup and popup != false}">
				<a title="${hint}" class="${classe} ${linkBotoes ? 'btn btn-sm btn-light' : ''}" <c:if test="${not empty accesskey}">accesskey="${accesskey}"</c:if>
					href="javascript:${linkConfirm}popitup('${url}');">${img}${title}</a>
			</c:when>
			<c:when test="${not empty ajax and ajax != false}">
				<span id="spanAjax_${idAjax}"> <a title="${hint}" class="${classe} btn btn-sm btn-light" <c:if test="${not empty accesskey}">accesskey="${accesskey}"</c:if>
			...
			<c:otherwise>
				<c:choose>
					<c:when test="${not empty linkConfirm}">
						<a title="${hint}" class="${classe} btn btn-sm btn-light" <c:if test="${not empty accesskey}">accesskey="${accesskey}"</c:if>
							href="javascript:${linkConfirm}location.href='${url}';">${img}${title}</a>
					</c:when>
					<c:otherwise>
						<a title="${hint}" class="${classe} btn btn-sm btn-light" <c:if test="${not empty accesskey}">accesskey="${accesskey}"</c:if> href="${url}">${img}${title}</a>
					</c:otherwise>
				</c:choose>
			</c:otherwise>
		</c:choose>
