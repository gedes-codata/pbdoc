...
public class OrgaoUsuarioController extends SigaSelecionavelControllerSupport<CpOrgaoUsuario, DaoFiltroSelecionavel> {
	...
	public OrgaoUsuarioController(HttpServletRequest request, HttpServletResponse response, Result result, SigaObjects so, EntityManager em) {
	...
	public void lista(Integer paramoffset, String nome) throws Exception {
		...
		if (!"ZZZ".equalsIgnoreCase(getTitular().getOrgaoUsuario().getSigla())) {
	...
	public void edita(final Long id) throws Exception {
		if (id != null) {
			...
			result.include("siglaOrgaoUsuarioCompleta", orgaoUsuario.getSiglaCompleta());
		}

		List<DpPessoa> listaPessoa = CpDao.getInstance().consultarPorMatriculaEOrgao(null, id, Boolean.FALSE,
				Boolean.FALSE);
	...
	public void editarGravar(final Long id, final String nmOrgaoUsuario, final String siglaOrgaoUsuario,
			final String siglaOrgaoUsuarioCompleta, final String acao) throws Exception {
		...
		if (!siglaOrgaoUsuario.matches("[a-zA-Z]{1,5}"))
			throw new AplicacaoException("Sigla Abreviada do órgão inválida");
		...
		if ((orgaoUsuario != null && !orgaoUsuario.getIdOrgaoUsu().equals(id))
				|| (orgaoUsuario != null && orgaoUsuario.getIdOrgaoUsu().equals(id) && acao.equalsIgnoreCase("i"))) {
		...
		if (orgaoUsuario != null && !orgaoUsuario.getSiglaOrgaoUsu().equalsIgnoreCase(siglaOrgaoUsuario)
				&& ((!orgaoUsuario.getIdOrgaoUsu().equals(id) && acao.equalsIgnoreCase("a"))
		...
		if (orgaoUsuario != null && acao.equalsIgnoreCase("i") && orgaoUsuario.getIdOrgaoUsu().equals(id)
				&& orgaoUsuario.getSiglaOrgaoUsu().equalsIgnoreCase(siglaOrgaoUsuario)
				&& orgaoUsuario.getNmOrgaoUsu().equals(nmOrgaoUsuario)) {
			throw new AplicacaoException("ID já cadastrado para outro órgão");
		...
		if (orgaoUsuario == null || orgaoUsuario.getIdOrgaoUsu() == null) {
			...
			orgaoUsuario.setSiglaCompleta(Texto.removerEspacosExtra(siglaOrgaoUsuarioCompleta.toUpperCase()).trim());
		...
		if (id  == null) {
			result.include("mensagemUsuario", "Órgão cadastrado com sucesso");
		} else {
			result.include("mensagemUsuario", "Órgão alterado com sucesso");
		}
		result.forwardTo(this).lista(0, "");
