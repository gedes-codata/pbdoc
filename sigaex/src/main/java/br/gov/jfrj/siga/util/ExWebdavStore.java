...
public class ExWebdavStore implements IWebdavStore {
	...
	public StoredObject getStoredObject(ITransaction t, String uri) {
		...
		if (StringUtils.EMPTY.equals(uri) || "/".equals(uri) || uri.endsWith("/") || uri.split("/").length == 2) {
			StoredObject o = new StoredObject();
			if (ret != null)
				dt = ret.mob.getDoc().getData();
			o.setFolder(true);
	...
	public long setResourceContent(ITransaction t, String uri, InputStream content, String contentType,
		...
		try {
			...
			Ex.getInstance().getBL()
				.anexarArquivoAuxiliar(cadastrante, cadastrante.getLotacao(), mob, null, titular,ctx.nmArq, titular, lotaTitular, ab);
		} catch (Exception e) {
