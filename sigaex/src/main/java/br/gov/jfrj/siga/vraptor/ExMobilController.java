...
public class ExMobilController extends ExSelecionavelController<ExMobil, ExMobilDaoFiltro> {
	...
	public ExMobilController(HttpServletRequest request, HttpServletResponse response, Result result, SigaObjects so, EntityManager em) {
...
//		System.out.println("Finalizou rotina");
	...
	public void aBuscar(final String sigla, final String popup, final String primeiraVez, final String propriedade,
			final Integer postback, final int apenasRefresh, final Long ultMovIdEstadoDoc, final int ordem,
			final int visualizacao, final Integer ultMovTipoResp, final DpPessoaSelecao ultMovRespSel,
			final DpLotacaoSelecao ultMovLotaRespSel, final Long orgaoUsu, final Long idTpDoc, final String dtDocString,
			final String dtDocFinalString, final Long idTipoFormaDoc, final Integer forma, final Long idMod,
			final String anoEmissaoString, final String numExpediente, final String numExtDoc,
			final CpOrgaoSelecao cpOrgaoSel, final String numAntigoDoc, final DpPessoaSelecao subscritorSel,
			String nmSubscritorExt, final Integer tipoCadastrante, final DpPessoaSelecao cadastranteSel,
			final DpLotacaoSelecao lotaCadastranteSel, final Integer tipoDestinatario,
			final DpPessoaSelecao destinatarioSel, final DpLotacaoSelecao lotacaoDestinatarioSel,
			final CpOrgaoSelecao orgaoExternoDestinatarioSel, final String nmDestinatario,
			final ExClassificacaoSelecao classificacaoSel, final String descrDocument, final String fullText,
		...
		builder.setPostback(postback).setUltMovTipoResp(ultMovTipoResp).setUltMovRespSel(ultMovRespSel)
				.setUltMovLotaRespSel(ultMovLotaRespSel).setOrgaoUsu(orgaoUsu).setIdTpDoc(idTpDoc)
				.setCpOrgaoSel(cpOrgaoSel).setSubscritorSel(subscritorSel).setTipoCadastrante(tipoCadastrante)
				.setCadastranteSel(cadastranteSel).setLotaCadastranteSel(lotaCadastranteSel)
				.setTipoDestinatario(tipoDestinatario).setDestinatarioSel(destinatarioSel)
				.setLotacaoDestinatarioSel(lotacaoDestinatarioSel)
				.setOrgaoExternoDestinatarioSel(orgaoExternoDestinatarioSel).setClassificacaoSel(classificacaoSel)
				.setOffset(offset);
...
//					+ (System.currentTimeMillis() - tempoIni));
		...
		if (!SigaExProperties.isConsultarFolhaDeRosto()) {
			...
			for (ExTipoDocumento i : l) {
				if (i.getId() == ExTipoDocumento.TIPO_DOCUMENTO_INTERNO_FOLHA_DE_ROSTO
						|| i.getId() == ExTipoDocumento.TIPO_DOCUMENTO_EXTERNO_FOLHA_DE_ROSTO)
					continue;
				l2.add(i);
			}
			l = l2;
	...
	public Download exportarCsv(final String popup, final String primeiraVez, final String propriedade,
			final Integer postback, final int apenasRefresh, final Long ultMovIdEstadoDoc, final int ordem,
			final int visualizacao, final Integer ultMovTipoResp, final DpPessoaSelecao ultMovRespSel,
			final DpLotacaoSelecao ultMovLotaRespSel, final Long orgaoUsu, final Long idTpDoc, final String dtDocString,
			final String dtDocFinalString, final Long idTipoFormaDoc, final Long idFormaDoc, final Long idMod,
			final String anoEmissaoString, final String numExpediente, final String numExtDoc,
			final CpOrgaoSelecao cpOrgaoSel, final String numAntigoDoc, final DpPessoaSelecao subscritorSel,
			String nmSubscritorExt, final Integer tipoCadastrante, final DpPessoaSelecao cadastranteSel,
			final DpLotacaoSelecao lotaCadastranteSel, final Integer tipoDestinatario,
			final DpPessoaSelecao destinatarioSel, final DpLotacaoSelecao lotacaoDestinatarioSel,
			final CpOrgaoSelecao orgaoExternoDestinatarioSel, final String nmDestinatario,
			final ExClassificacaoSelecao classificacaoSel, final String descrDocumento, final String fullText,
		...
		List lista = dao().consultarPorFiltroOtimizado(flt, builder.getOffset(), getTamanho(), getTitular(),
				getLotaTitular());

		InputStream inputStream = null;
		StringBuffer texto = new StringBuffer();
		texto.append(";Responsável pela Assinatura;;;Responsável pela situação atual"
				+ System.getProperty("line.separator"));
		texto.append("Número;Unidade;Usuário;Data;Unidade;Usuário;Data;Situação;Documento;Descrição"
				+ System.getProperty("line.separator"));
		...
		for (Object object : lista) {
			...
			if (ma.getDpLotacaoIni() != null && ma.getDpLotacaoIni().getLotacaoAtual() != null
					&& ma.getDpLotacaoIni().getLotacaoAtual().getSigla() != null) {
			...
			if (SigaBaseProperties.getString("siga.local") != null
				...
				descricao = Ex.getInstance().getBL().descricaoSePuderAcessar(e, getTitular(),
	...
	public void aListar(final String popup, final String primeiraVez, final String propriedade, final Integer postback,
			final int apenasRefresh, final Long ultMovIdEstadoDoc, final int ordem, final int visualizacao,
			final Integer ultMovTipoResp, final DpPessoaSelecao ultMovRespSel, final DpLotacaoSelecao ultMovLotaRespSel,
			final Long orgaoUsu, final Long idTpDoc, final String dtDocString, final String dtDocFinalString,
			final Long idTipoFormaDoc, final Long idFormaDoc, final Long idMod, final String anoEmissaoString,
			final String numExpediente, final String numExtDoc, final CpOrgaoSelecao cpOrgaoSel,
			final String numAntigoDoc, final DpPessoaSelecao subscritorSel, String nmSubscritorExt,
			final Integer tipoCadastrante, final DpPessoaSelecao cadastranteSel,
			final DpLotacaoSelecao lotaCadastranteSel, final Integer tipoDestinatario,
			final DpPessoaSelecao destinatarioSel, final DpLotacaoSelecao lotacaoDestinatarioSel,
			final CpOrgaoSelecao orgaoExternoDestinatarioSel, final String nmDestinatario,
			final ExClassificacaoSelecao classificacaoSel, final String descrDocumento, final String fullText,
			final Long ultMovEstadoDoc, final Integer paramoffset) throws Exception {
		assertAcesso("");
		...
		if (primeiraVez == null || !primeiraVez.equals("sim")) {
			...
			if (anoEmissaoString != null && !anoEmissaoString.equals("0")) {

				executaFiltro(builder, flt);

			} else if ((destinatarioSel.getSigla() == null && lotacaoDestinatarioSel.getSigla() != null
					&& descrDocumento == null)
					|| (destinatarioSel.getSigla() != null && lotacaoDestinatarioSel.getSigla() == null
							&& descrDocumento == null)
					|| (destinatarioSel.getSigla() == null && lotacaoDestinatarioSel.getSigla() == null
							&& descrDocumento != null)
					|| (destinatarioSel.getSigla() != null && lotacaoDestinatarioSel.getSigla() == null
					...
					|| (destinatarioSel.getSigla() == null && lotacaoDestinatarioSel.getSigla() != null
							&& descrDocumento != null)) {
				executaFiltro(builder, flt);
			} else if ((cadastranteSel.getSigla() == null && lotaCadastranteSel.getSigla() != null
					&& descrDocumento == null)
					|| (cadastranteSel.getSigla() != null && lotaCadastranteSel.getSigla() == null
							&& descrDocumento == null)
					|| (cadastranteSel.getSigla() == null && lotaCadastranteSel.getSigla() == null
							&& descrDocumento != null)
					|| (cadastranteSel.getSigla() != null && lotaCadastranteSel.getSigla() == null
				...
				if ((ultMovRespSel.getSigla() == null && ultMovLotaRespSel.getSigla() == null && descrDocumento == null)
						&& (destinatarioSel.getSigla() == null && lotacaoDestinatarioSel.getSigla() == null
								&& descrDocumento == null)
						&& (cadastranteSel.getSigla() == null && lotaCadastranteSel.getSigla() == null
								&& descrDocumento == null)) {
					errormsg = "Preenchimento obrigatório mínimo de um dos campos abaixo: <br/>- Usuário/Unidade <br/>- Descrição <br/>- Ano de Emissão <br/>- Cadastrante <br/>- Destinatário";
				} else {
					if ((ultMovRespSel.getSigla() == null && ultMovLotaRespSel.getSigla() != null
							&& descrDocumento == null)
							|| (ultMovRespSel.getSigla() != null && ultMovLotaRespSel.getSigla() == null
									&& descrDocumento == null)
							|| (ultMovRespSel.getSigla() == null && ultMovLotaRespSel.getSigla() == null
									&& descrDocumento != null)
							|| (ultMovRespSel.getSigla() != null && ultMovLotaRespSel.getSigla() == null
		...
		result.include("currentPageNumber",

				calculaPaginaAtual(paramoffset));
		result.include("idTipoFormaDoc", idTipoFormaDoc);
		result.include("idFormaDoc", idFormaDoc);
		result.include("idMod", idMod);
		result.include("errormsg", errormsg);
	...
	private void executaFiltro(final ExMobilBuilder builder, final ExMobilDaoFiltro flt) {
		setTamanho(dao().consultarQuantidadePorFiltroOtimizado(flt, getTitular(), getLotaTitular()));
		setItens(dao().consultarPorFiltroOtimizado(flt, builder.getOffset(), getItemPagina(), getTitular(),
				getLotaTitular()));
	}

	final static Pattern preprocessadorPatternSelecao = Pattern.compile("^([\\w_]+)(_[a-z]+Sel.)([\\w_]+)$");
	...
	protected ExMobilDaoFiltro createDaoFiltro() {
		...
		if (flt.getClassificacaoSelId() != null)
			flt.setClassificacaoSelId((daoClassificacao(flt.getClassificacaoSelId())).getIdInicial());
	...
	private List<ExFormaDocumento> getTodasFormasDocPorTipoForma(final Long idTipoFormaDoc) {
		...
		formasDoc.addAll(bl.obterFormasDocumento(bl.obterListaModelos(null, null, false, false, null, null, false,
				getTitular(), getLotaTitular(), false), null, tipoForma));
		return formasDoc;
	}
	...
	private List<ExModelo> getModelos(final Long idFormaDoc) {
		...
		return Ex.getInstance().getBL().obterListaModelos(null, forma, false, false, null, "Todos", false, getTitular(),
	...
	public Selecionavel selecionarVerificar(Selecionavel sel) throws AplicacaoException {
		...
		// Edson: Se a via, volume ou documento inteiro tiver sido eliminado(a), não
		// retorna nada.
		if (mob.isEliminado())
			return null;
		...
		if (mob.isGeral() && mob.getDoc().isExpediente()) {
			...
			for (ExMobil m : mob.getDoc().getExMobilSet()) {
				...
				if (mov != null && mov.getResp() != null && mov.getResp().equivale(super.getTitular())) {
					return m;
				}
			}
			...
			for (ExMobil m : mob.getDoc().getExMobilSet()) {
				if (m.isGeral() || m.isEliminado())
					continue;
				ExMovimentacao mov = m.getUltimaMovimentacaoNaoCancelada();
				if (mov != null && mov.getLotaResp() != null && mov.getLotaResp().equivale(super.getLotaTitular())) {
					return m;
				}
			}
		}
		...
		if (mob.isGeral() && mob.getDoc().isProcesso()) {
			...
			//
			ExMobil m = mob.getDoc().getUltimoVolume();
			if (m != null) {
				ExMovimentacao mov = m.getUltimaMovimentacaoNaoCancelada();
				if (mov == null) {
					return m;
				}
				if (mov.getLotaResp() != null && mov.getLotaResp().equivale(super.getLotaTitular())) {
					return m;
				}
				if (mov.getResp() != null && mov.getResp().equivale(super.getTitular())) {
					return m;
				}
			}
		}
