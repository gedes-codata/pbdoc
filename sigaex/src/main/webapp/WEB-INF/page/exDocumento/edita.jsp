...
<div class="container-fluid">
		<c:if test="${not empty mensagemUsuario}">
			<div class="row" id="row-mensagem-retorno">
				<div class="col">
					<p id="mensagem-retorno" class="alert alert-danger mb-3 mb-0">${mensagemUsuario}</p>
					<script>
						setTimeout(function() {
			...
			<form id="frm" name="frm" theme="simple" method="post" enctype="multipart/form-data" class="mb-0">
				...
				<div class="row">
												...
												<c:forEach items="${hierarquiaDeModelos}" var="item">
													<li class="dropdown-item" data-value="${item.value}"
														...
														<c:if test="${item.group}">
															<a href="#" class="d-block">${item.text}</a>
														</c:if>
														<c:if test="${!item.group}">
															<a href="#" class="d-block">${item.text}<small class="pl-2 text-muted">${item.keywords}</small></a>
														</c:if>
													</li>
												</c:forEach>
											</ul>
					...
					<div class="row ${((exDocumentoDTO.tiposDocumento).size() != 1 or (exDocumentoDTO.tipoDocumento != 'interno_capturado' and podeEditarData) or (exDocumentoDTO.listaNivelAcesso.size() != 1) or (!exDocumentoDTO.eletronicoFixo))? '': 'd-none'}">
						...
						<div class="col-sm-2 ${exDocumentoDTO.eletronicoFixo? 'd-none': ''} ${hide_only_GOVPB}">
							<div class="form-group">
				...
				<div class="row">
					...
					<div class="col-sm-4 ${hide_only_GOVPB} ">
						<div class="form-group">
				...
				<input id="destinatario" type="hidden" name="campos" value="tipoDestinatario" />
						...
						<div class="form-group">
							...
							<c:choose>
								...
								<c:otherwise>
									...
									<label>&nbsp;&nbsp;&nbsp;</label>
									<input type="text" id="exDocumentoDTO.nmDestinatario" name="exDocumentoDTO.nmDestinatario" size="80" maxLength="256" value="${exDocumentoDTO.nmDestinatario}" class="form-control" />
								</c:otherwise>
							</c:choose>
							</siga:span>
						</div>
				...
				<div class="row ${hide_only_GOVSP}">
					<div class="col-sm-5">
						<div class="form-group">
							<label>Descrição do Assunto/Classificação</label>
				...
				<div class="row mt-4">
					<div class="col-sm-8">
						<button type="button" onclick="javascript: gravarDoc(); return false;" name="gravar" class="btn btn-primary" accesskey="g" title="Apenas grava o documento podendo continuar a Edição"><u>G</u>ravar</button> 
						<button type="button" onclick="javascript: gravarDocEAssinar(); return false;" name="finalizareGravar" class="btn btn-primary" accesskey="f" title="Finalizar documento em definitivo e em seguida realizar assinatura digital"><u>F</u>inalizar e Assinar</button>
						<c:if test='${exDocumentoDTO.tipoDocumento == "interno"}'>
							<c:if test="${not empty exDocumentoDTO.modelo.nmArqMod or exDocumentoDTO.modelo.conteudoTpBlob == 'template/freemarker'}">
								...
								<button type="button" name="ver_doc_pdf" onclick="javascript: popitup_documento(true); return false;" class="btn btn-info ${hide_only_GOVPB}" accesskey="i"><fmt:message key="documento.btn.ver.impressao2"/></button>
								...
								<button type="button" name="voltar" onclick="javascript:window.location.href='${pageContext.request.contextPath}/app/expediente/doc/exibir?sigla=${exDocumentoDTO.sigla}'" 
								class="btn btn-info" accesskey="r" class="btn btn-primary mr-2" role="button">Volta<u>r</u></button>
							</c:if>
						</c:if>
					</div>
				</div>
