...
<siga:pagina titulo="Mesa Virtual">
				...
				<table class="table table-sm table-borderless">
					<tbody>
						...
						<tr v-if="f.grupoExibir" class="table-head">
							...
							<th class="d-none d-md-block">Assunto</th>
						...
						<tr v-bind:class="{odd: f.odd}">
							...
							<td><a :href="'expediente/doc/exibir?sigla=' + f.codigo" :title="f.sigla">{{numeroDocumentoSemVolume(f)}}</a><span class="d-inline d-md-none"> - {{f.descr}}</span></td>
	...
	var demo = new Vue({
		  ...
		  methods: {
			...
			numeroDocumentoSemVolume: function(item) {
				// Sem sigla
				if (!item.sigla) {
					return item.sigla;
				}

				var split = item.sigla.split('/');
				...
				var lastIndexOfHyphen = last.lastIndexOf('-');

				// Não foi possível identificar um hífen "-"
				if (lastIndexOfHyphen < 1) {
					return item.sigla;
				}

				// Sigla sem código de volume
				return item.sigla.substring(0, item.sigla.lastIndexOf('-'));
			}
