...
<div class="col pt-2 pb-2">
	...
	<div class="form-check form-check-inline mr-2 d-none">
		<input class="form-check-input" type="checkbox" accesskey="c" name="ad_password_0" id="ad_password_0" checked readonly />
		<label class="form-check-label" for="ad_password_0"><u>C</u>om Senha</label>
	</div>

	<c:if test="${not empty juntarAtivo}">
		<div class="form-check form-check-inline mr-2 ${hide_only_GOVSP}">
			<input class="form-check-input" type="checkbox" accesskey="j" name="ad_juntar_0" id="ad_juntar_0"
				<c:if test="${juntarAtivo}">checked</c:if>
				<c:if test="${juntarFixo}">disabled</c:if>
			/>
			<label class="form-check-label" for="ad_juntar_0"><u>J</u>untar</label>
		</div>
	</c:if>

	<c:if test="${not empty tramitarAtivo}">
		<div class="form-check form-check-inline mr-2 ">
			<input class="form-check-input" type="checkbox" accesskey="t" name="ad_tramitar_0" id="ad_tramitar_0"
				<c:if test="${tramitarAtivo}">checked</c:if>
				<c:if test="${tramitarFixo}">disabled</c:if>
			/>
			<label class="form-check-label" for="ad_tramitar_0"><u>T</u>ramitar</label>
		</div>
	</c:if>
	<button type="button" name="voltar" onclick="javascript:window.location.href='${pageContext.request.contextPath}/app/expediente/doc/editar?sigla=${sigla}'" class="btn btn-info ${hide_only_TRF2}" accesskey="r"
	class="btn btn-primary mr-2" role="button">Volta<u>r</u></button>
