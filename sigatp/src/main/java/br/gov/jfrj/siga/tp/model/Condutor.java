...
//	}
	...
	public static List<Condutor> listarDisponiveis(String dataSaida, Long idMissao, Long idOrgao, PerguntaSimNao inicioRapido) throws Exception {
		...
		qrl.append("SELECT c FROM Condutor c " + " WHERE DATE(c.dataVencimentoCNH) > DATE(" + dataFormatadaOracle + ")" + "  AND c.cpOrgaoUsuario.id in  " + "(SELECT cp.id FROM CpOrgaoUsuario cp"
